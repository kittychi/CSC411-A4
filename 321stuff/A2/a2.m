...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  dCdZh = class_prob - data.targets; %(classes x datacases)
  dCdWh = dCdZh * transpose(hid_output) ; % class X hid


  dZidWi = data.inputs; % inp X d cases
  dYidZi = hid_output .* (1 - hid_output); % hid x d cases
  dZhdYi = model.hid_to_class; % classes x hid

  dCdWi = ((transpose(dZhdYi) * dCdZh) .* dYidZi) * transpose(dZidWi);

  %ret.input_to_hid = dCdWi;
  %ret.hid_to_class = dCdWh;  
  ret.input_to_hid = (model.input_to_hid*wd_coefficient + dCdWi/n);
  ret.hid_to_class = (model.hid_to_class*wd_coefficient + dCdWh/n);
end
